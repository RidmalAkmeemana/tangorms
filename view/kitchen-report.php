...
while ($row = $orderResult->fetch_assoc()) {
    ...
    $reason = empty($row['reason']) ? 'N/A' : $row['reason'];

    // Save starting X/Y
    $x = $fpdf->GetX();
    $y = $fpdf->GetY();

    // Row heights: track max height of multi-cells
    $fpdf->SetXY($x, $y); $fpdf->Cell(33, $cellHeight, $row['receipt_no'], 1, 0, "C");
    $fpdf->SetXY($x + 33, $y); $fpdf->Cell(35, $cellHeight, $row['customer_name'], 1, 0, "C");
    $fpdf->SetXY($x + 28 + 40, $y); $fpdf->Cell(18, $cellHeight, $row['order_status'], 1, 0, "C");
