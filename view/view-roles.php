                        ...
                        <table class="table table-bordered table-hover align-middle text-center table-striped" id="usertable">
                            ...
                            <tbody>
                                <?php while ($roleRow = $roleResult->fetch_assoc()): ?>
                                    <?php
                                    $role_id = base64_encode($roleRow["role_id"]);
                                    $status = $roleRow["role_status"] == 1 ? "Active" : "Deactive";
                                    $status_class = $roleRow["role_status"] == 1 ? "badge bg-success" : "badge bg-danger";
                                    ?>
                                    <tr>
                                        <td class="text-start pe-3"><?= htmlspecialchars($roleRow["role_name"]) ?></td>
                                        <td>
                                            <span class="<?= $status_class ?> px-3 py-1"><?= $status ?></span>
                                        </td>
                                        <td>
                                            <a href="../controller/role_controller.php?status=<?= $roleRow["role_status"] == 1 ? 'deactivate' : 'activate' ?>&role_id=<?= $role_id ?>"
                                                class="btn btn-sm <?= $roleRow["role_status"] == 1 ? 'btn-danger' : 'btn-success' ?>">
                                                <i class="fa <?= $roleRow["role_status"] == 1 ? 'fa-times' : 'fa-check-circle' ?>"></i>
                                                <?= $roleRow["role_status"] == 1 ? 'Deactivate' : 'Activate' ?>
                                            </a>
                                        </td>
                                        <td>
